    ...
    // tabs不需要保存在localStorage
    {
      ...
      serializer: {
        ...
        deserialize(value: string) {
          ...
          const visitHistory =
            raw instanceof Stack
              ? raw
              : createStack<string>(true, MAX_VISIT_HISTORY);
          if (!(raw instanceof Stack) && Array.isArray(raw?.items)) {
            visitHistory.push(...raw.items);
          }
          return {
